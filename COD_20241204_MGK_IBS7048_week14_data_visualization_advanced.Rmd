---
title: "COD_20241204_MGK_IBS7048_week14_data_visualization_advanced"
author: "Minsik Kim"
date: "2024-12-04"
output:
        rmdformats::downcute:
...
mpg %>% 
        ggplot(aes(x = cty, y = hwy, col = as.factor(cyl))) +
        geom_point() +
        xlab("City mileage (MPG)") +
        ylab("Highway mileage (MPG)")
...
mpg %>% 
        ggplot(aes(x = cty, y = hwy, col = manufacturer)) +
        geom_point() + 
        ggrepel::geom_text_repel(aes(label = model), max.overlaps = 100, ) +
        xlab("City mileage (MPG)") +
        ylab("Highway mileage (MPG)") +
        theme(axis.title.y = element_markdown(family = "serif", size = 15))
